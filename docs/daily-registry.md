...
Provisional daily eligibility requires a canonical site/docs surface, mainnet/beta lifecycle, positive deployed-on-chain qualification, and a sourced verified own deployment on Robinhood Chain. This changes collection eligibility only: identity, independent review, coverage, scores and publication remain untouched. Relevance values must be dated within seven days and token liquidity must belong to the subject's own token; a stale display value may justify recovery, never a fresh public label.
