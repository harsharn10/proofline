...
QUALITY BAR (the owner's words after reading the automatic wire on 2026-09-04: "literally just sharing tweets by small accounts on projects no one cares about; not interesting, not a catch-up, not a Bloomberg-level alert"): every message must pass the question "would a subscriber who follows Robinhood Chain act on this or be surprised by it". Concretely: Talk counts only from accounts in content/accounts.yaml with tier top, or with at least 100,000 followers recorded on the receipt, and only when it coincides with a number moving the same day; project posts count only when tagged listing, integration, partnership, audit, launch-date, whitelist or mint with a receipt that is not the project's own account; every alert carries at least one number and the name's TL;DR; the daily brief opens with the one thing that mattered most that day. Acceptance: backtest the rules over the last seven days of committed data (content/pulled/history/*.jsonl, content/feed, content/changelog, build/derived.json at each day) and paste every message they would have sent, day by day, into the PR body under "## Backtest"; the owner reads that list before the merge, and a rule that produces a message he would not have wanted is tightened until it does not. Fewer messages is always the safer failure.
```
