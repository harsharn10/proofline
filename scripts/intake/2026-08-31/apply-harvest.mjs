// Task 5 — merge scripts/harvest-data.mjs into content/. Run after `npm run seed` (which creates the
...
//
//   node scripts/apply-harvest.mjs
//
// Per census slug:
...
for (const c of census) {
  ...
  if (!harvest) { console.error(`no harvest data for ${c.slug} — add an entry to scripts/harvest-data.mjs (empty lists are fine)`); process.exit(1); }
  ...
  if (feed) { await writeYaml(`content/feed/${c.slug}.yaml`, feed, `# Generated by scripts/apply-harvest.mjs from scripts/harvest-data.mjs — dated, attributed items; numbers are what an account posted.\n`); feedFiles++; }
}
